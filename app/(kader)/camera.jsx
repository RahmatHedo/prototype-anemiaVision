...
import { View, Text, StyleSheet, TouchableOpacity, Modal, ScrollView, ActivityIndicator, Alert, Dimensions, Image, TextInput } from 'react-native';
  ...
  const handleCapture = async () => {
    if (cameraRef.current) {
      try {
        ...
        // Quality check
        const quality = await verifyImageQuality(photo.uri);
        if (!quality.success) {
        ...
        const cnnRes = await runPhase1CNN(photo.uri);
        setCnnResult(cnnRes);
        ...
        <ScrollView style={styles.resultContainer} contentContainerStyle={styles.resultScroll}>
          ...
          <View style={styles.resultDetailsCard}>
            ...
            {cnnResult.isAnemic && (
              <>
                <View style={styles.detailRow}>
                  <Text style={styles.detailName}>Usia Siswi</Text>
